# Hello, I'm Marwa Akeel 👋

Welcome to my GitHub profile! I'm excited to share my journey as I grow my skills and work on projects that inspire me. Here's a little about me:

🔭 **I’m currently working on:**
- Building my career and enhancing my professional skills 😄
- Exploring projects that combine creativity and technology

💻 **Featured Projects:**
- **Al Tayer Motors Website Clone:** [View Live Site](https://marwaakeel29.github.io/) • [View Repository](https://github.com/MarwaAkeel29/Frontend-Automotive-Website) | Designed and developed a responsive front-end interface using HTML, CSS, and vanilla JS.
- **Frontend Web Dev Portfolio:** [View Repository](https://github.com/MarwaAkeel29/Frontend-Web-Dev-Portfolio) | A comprehensive 6-chapter collection of responsive web layouts, advanced CSS styling, and interactive JavaScript applications.
- **Grimoire - Harry Potter API Explorer:** [View Repository](https://github.com/MarwaAkeel29/Grimoire-HarryPotter-API-GUI) | A Python Tkinter desktop application that dynamically fetches and displays Wizarding World data using external REST APIs.
- **Python Tkinter GUI Collection:** [View Repository](https://github.com/MarwaAkeel29/Python-Tkinter-GUI-Collection) | A suite of three interactive desktop apps (Space Math Game, Hacker Joke Terminal, and a Student CRUD Database Manager).
- **Zero-Knowledge Password Vault:** [View Repository](https://github.com/marwaakeel29/Zero-Knowledge-Password-Vault) | A highly secure, offline password manager utilizing PBKDF2 hashing and AES-256 data encryption.

🌱 **I’m currently learning:**
- Cybersecurity at Bath Spa University, with a heavy focus on ethical hacking, secure coding, network security, and digital forensics.

💬 **About Me:**
- I’m originally from India, but I was born and raised in the UAE.
- I love staying busy with creative activities, which is where I feel most inspired.

⚡ **Fun Fact:**
- I'm a multifaceted individual with a knack for creativity and problem-solving! 
- My interests outside of tech span:
  - 🖋️ Calligraphy
  - 🎨 Drawing
  - 🍰 Baking
  - 🔨 Engraving
  - 📖 Exploring new courses and skills (latest included!)

📫 **Connect with Me:**
- **LinkedIn:** [Marwa Akeel](https://www.linkedin.com/in/marwaakeel)
- **Email:** [marwaakeel2020@gmail.com](mailto:marwaakeel2020@gmail.com)
  
